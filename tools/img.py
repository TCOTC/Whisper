...
# 以脚本所在目录为基准，定位到主题根目录下的 image-material
_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_MATERIAL = os.path.join(_ROOT, 'image-material')


def Image_Merge(left_image_path, right_image_path, output_path, is_save=True):
    try:
        ...
        # 对 vi 图片作为背景，创建一个绘图对象
        draw = ImageDraw.Draw(image1)

        # 绘制一个斜对角线的三角形，即左上角的三角形，设置透明度为 0
...
# 左上角为明亮主题，右下角为暗黑主题，输出到相同目录
left_image_path = os.path.join(_MATERIAL, 'preview-light.png')
right_image_path = os.path.join(_MATERIAL, 'preview-dark.png')
output_path = os.path.join(_MATERIAL, 'preview.png')
Image_Merge(left_image_path, right_image_path, output_path, is_save=True)
